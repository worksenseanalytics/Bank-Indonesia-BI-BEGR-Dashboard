...
// Define the logic to completely rewrite dataUtils.ts based on the 94 columns.
const TS_FILE = path.join('c:\\Users\\IKHSAN KAMAL\\Downloads\\remix_-bi\\src\\data\\dataUtils.ts');

const newContent = `import { mockData } from "./mockData";
...
export const getPengaturanData = () => {
  const defaults = { 
    ...
    adminUser: "Divisi Pengembangan Budaya Kerja",
    targetMaturity: 4.00
  };
...
export function getCmlCategory(score: number): "Aligned" | "Engaged" | "Enable" | "Empower" {
  if (score < 3.00) return "Aligned";
  if (score < 3.75) return "Engaged";
  if (score < 4.30) return "Enable";
  return "Empower";
}
...
console.log('Successfully rewrote dataUtils.ts');
